...
class collMdl : public animation, public BLEND {
	public:
    	collMdl()												{ min_p = 1; max_p = 10; show_time = 8; cl = cr = 0; l = r = ml = mr = 0; mode = wait = 0; }
    	virtual void	init(void);
    	virtual void	show(void);
	private:
    	void			newColors(void);
...
class swing : public animation, public CRAWL {
	public:
    	swing()													{ min_p = 2; max_p = 9; show_time = 10; }
    	virtual void 	init(void);
    	virtual void 	show(void);
	private:
    	int16_t			len			= 0;
...
class swingSingle : public animation {
	public:
    	swingSingle()											{ min_p = 4; max_p = 10; show_time = 10; }
    	virtual void 	init(void);
    	virtual void 	show(void);
	private:
    	COLOR			color		= 0;
...
class theatChase: public animation {
	public:
		theatChase(void)										{ min_p = 6; max_p = 16; show_time = 5; }
    	virtual void	init(void);
    	virtual void	show(void);
	private:
    	uint8_t			w				= 0;					// color wheel index
    	uint8_t			color_period	= 0;					// color period shift
...
class dropFade: public animation, public BRGTN {
	public:
		dropFade(void)											{ min_p = 6; max_p = 16; show_time = 6; }
		virtual void	init(void);
		virtual void	show(void);
	private:
		void			newDrop(void);
		bool			isActiveDrop(uint16_t pos);
		struct	c_drop {
			uint16_t	pos;									// Rising pixel position
			uint8_t		w;										// The wheel index of the pixel color
			bool		remove;									// The pixel should be deleted
		}				drop[15];

		uint8_t			active_drops	= 0;					// The number of the active drops
		const uint8_t	max_drops		= 15;
};
